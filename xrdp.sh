...
if [[ "$XDG_SESSION_TYPE" == *"tty"* ]]; then
    ...
    cnt=$(find /usr/share/xsessions -maxdepth 1 -type f | wc -l)
    echo "$cnt"
    ...
    if [ "$cnt" -gt "1" ]; then
        ...
        mapfile -t desk < <(find /usr/share/xsessions -maxdepth 1 -type f -exec basename {} .desktop \;)

        select menu in "${desk[@]}"; do
...
PrepOS() {
    ...
    if [[ "$version" == *"Mint"* ]]; then
        ucodename=$(grep UBUNTU_CODENAME /etc/os-release | awk -F"=" '{print $2}')
    fi

    #--Additional Code for LMDE - Used To Configure proper Repositories
    if [[ "$version" == *"LMDE"* ]]; then
        ucodename=$(grep DEBIAN_CODENAME /etc/os-release | awk -F"=" '{print $2}')
    fi
...
install_prereqs() {
    ...
    if sudo apt-get -y install libmp3lame-dev curl libfuse-dev libx11-dev libxfixes-dev libssl-dev libpam0g-dev libtool libjpeg-dev flex bison gettext autoconf libxml-parser-perl xsltproc libxrandr-dev python3-libxml2 nasm pkg-config git intltool checkinstall; then
...
get_binaries() {
    ...
    if [ "$devcode" = "yes" ]; then
        ...
        if [[ $LastReleaseXrdp =~ v0.9 ]]; then
            LastReleaseXrdp=0.10
...
create_polkit() {
    ...
    if (( $(echo "$chkPkv > 0.106" | bc -l) )); then
...
fix_theme() {
    ...
    #need to check if this exists in file, if not use old school
...
snd_pipewire() {
    ...
    if apt-cache search pipewire-module-xrdp | grep -sq pipewire-module-xrdp; then
        echo
    ...
    #Build & Install
    cd ~/Downloads/pipewire-module-xrdp || exit
    ./bootstrap
    ./configure
    make
    sudo make install
...
EOF
    ...
    cd /tmp/pulseaudio-"$pulsever"* || exit
    PulsePath=$(pwd)
    ...
    cd pulseaudio-module-xrdp || exit
    ./bootstrap
